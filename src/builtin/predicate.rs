use crate::gc::{GcHeap, GcRef, new_bool, new_symbol, SchemeValue};
...
mod tests {
    ...
    fn test_number_q() {
        ...
        let sym_val = crate::gc::new_symbol(&mut heap, "foo");
    ...
    fn test_eq_q() {
        ...
        let sym1 = crate::gc::new_symbol(&mut heap, "foo");
        let sym2 = crate::gc::new_symbol(&mut heap, "foo");
        let result = eq_q(&mut heap, &[sym1, sym2]);
        assert!(result.is_ok());
        assert!(matches!(&result.unwrap().value, SchemeValue::Bool(true)));
        
        // Test with unequal symbols
        let sym3 = crate::gc::new_symbol(&mut heap, "bar");
        let sym4 = crate::gc::new_symbol(&mut heap, "baz");
        ...
        // Test with equal nil
        let nil1 = crate::gc::new_nil(&mut heap);
        let nil2 = crate::gc::new_nil(&mut heap);
        let result = eq_q(&mut heap, &[nil1, nil2]);
        assert!(result.is_ok());
        assert!(matches!(&result.unwrap().value, SchemeValue::Bool(true)));
        
        // Test with unequal nil
        let nil3 = crate::gc::new_nil(&mut heap);
    ...
    fn test_type_of() {
        ...
        // Test with nil
        let nil_val = crate::gc::new_nil(&mut heap);
