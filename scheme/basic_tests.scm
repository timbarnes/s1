...
(display "          === Testing More Vector Functions ===")
(newline)
(test-equal '#() (vector) "vector constructor with no args")
(test-equal '#(1 #t "hello") (vector 1 #t "hello") "vector constructor with mixed types")
(test-equal #(() () ()) (make-vector 3) "make-vector with only length")
(test-equal '#(b b b b) (vector-fill! (make-vector 4 'a) 'b) "vector-fill! replaces all elements")
(test-equal 0 (vector-length (make-vector 0)) "vector-length of zero-length vector")
(test-equal 3 (vector-ref #(1 2 3) 2) "vector-ref last element")
(define my-vec (vector 1 2 3))
(vector-set! my-vec 1 99)
(test-equal '#(1 99 3) my-vec "vector-set! modifies vector")
(test-equal '(1 2 3) (vector->list #(1 2 3)) "vector->list with non-empty vector")
(test-equal '() (vector->list #()) "vector->list with empty vector")
(test-equal '#(1 2 3) (list->vector '(1 2 3)) "list->vector with non-empty list")
(test-equal '#() (list->vector '()) "list->vector with empty list")
(define fill-vec (make-vector 4 'a))
(vector-fill! fill-vec 'b)
(test-equal '#(b b b b) fill-vec "vector-fill! replaces all elements")
;; The following expression should produce an error, but not panic the interpreter.
;; (vector-ref #() -1)